...
async function main() {
  ...
  if (!fs.existsSync(statePath)) {
    console.error('No Cengage session state found. Please run "npm run auth:cengage" first.');
    process.exit(1);
  }
  ...
  console.log('Navigating directly to your MATH 1014 WebAssign via the SSO link...');
  await page.goto('https://www.webassign.net/v4cgi/login.pl?pid=571417&eISBN=9780357128992&courseKey=WA-production-1607530', { waitUntil: 'load', timeout: 30000 });
  ...
  try {
    ...
    console.log('Warning: Did not completely reach the expected WebAssign dashboard URL.');
  }

  console.log('Looking for "Past Assignments" button...');
  try {
      // WebAssign past assignments button often has id or specific text
      await page.waitForSelector('button:has-text("Past Assignments")', { timeout: 10000 });
      await page.click('button:has-text("Past Assignments")');
      console.log('Clicked "Past Assignments" button');
  } catch (e) {
      console.log('Could not find "Past Assignments" button using text selector. Trying alternatives...');
      try {
  ...
  const screenshotPath = path.resolve(__dirname, '../.eclass-mcp/webassign-screenshot.png');
  await page.screenshot({ path: screenshotPath, fullPage: true });
  console.log(`Screenshot saved to ${screenshotPath}`);

  const dumpPath = path.resolve(__dirname, '../.eclass-mcp/webassign-dashboard.html');
