...
(async () => {
    ...
    try {
        ...
        if (relayStore) {
            ...
            const teacherKey = keys.find(k => k.toLowerCase().includes('teacher') && !k.toLowerCase().includes('search'));
            if (teacherKey) {
                console.log(`\nFound Teacher Entry: ${teacherKey}`);
                console.log(`Data Snippet: ${JSON.stringify(relayStore[teacherKey], null, 2).substring(0, 500)}...`);
            }
            
            const ratingKeys = keys.filter(k => k.toLowerCase().includes('rating'));
            console.log(`\nFound ${ratingKeys.length} Rating entries.`);
            if (ratingKeys.length > 0) {
                console.log(`First Rating Snippet: ${JSON.stringify(relayStore[ratingKeys[0]], null, 2).substring(0, 500)}...`);
            }

            // Dump a larger part of the store into a file for analysis
            const fullStoreJson = JSON.stringify(relayStore, null, 2);
            const fs = require('fs');
            fs.writeFileSync('.eclass-mcp/debug/rmp_relay_store_dump.json', fullStoreJson);
