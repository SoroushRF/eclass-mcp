...
async function testDeadlines() {
  ...
  try {
    ...
    console.log(`- Tool returned ${deadlines.length} items. Cache hit: ${meta?.hit}`);
    ...
    console.log(`📊 get_deadlines(month=${year}-${month}) returned ${monthItems.length} items. Cache hit: ${monthContent._cache?.hit}`);
    ...
    console.log(`📊 get_deadlines(range) returned ${rangeItems.length} items. Cache hit: ${rangeContent._cache?.hit}`);
