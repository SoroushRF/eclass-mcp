...
export class RMPClient {
    ...
    async searchTeachersWithDiagnostics(
        ...
        const terms = fallbackTerm && fallbackTerm !== normalizedName
        ...
        const direct = await this.searchTeachersAcrossSchools(query, terms, requestedSchoolIds, normalizedName, campus || null);
        ...
        if (campus && direct.length === 0) {
            ...
            diagnostics.note = probe.length > 0
    ...
    private async searchTeachersAcrossSchools(
        ...
        for (const schoolId of schoolIds) {
            for (const [index, term] of terms.entries()) {
                console.error(`[RMP] querying schoolId=${schoolId} term="${term}" attempt=${index + 1}/${terms.length}`);
                ...
                if (data?.errors?.length) {
                    const message = data.errors.map((err) => err.message || 'unknown RMP error').join('; ');
                    throw new Error(`RMP GraphQL errors: ${message}`);
                }

                const teachers = data?.data?.newSearch?.teachers?.edges || [];
                if (teachers.length === 0) {
                    console.error(`[RMP] no matches for schoolId=${schoolId} term="${term}"`);
                    continue;
                }
    ...
    async getTeacherDetails(teacherId: string): Promise<RMPTeacherDetails | null> {
        ...
        if (!teacher || teacher.__typename !== "Teacher") {
            console.error(`[RMP] teacher details missing or wrong type for teacherId=${teacherId}`);
            return null;
        }
    ...
    private async fetchGraphQL(query: string, payload: { operationName?: string; variables: any }): Promise<GraphQLResponse> {
        try {
            ...
            if (parsed.errors?.length) {
                console.error(`[RMP] GraphQL response errors: ${JSON.stringify(parsed.errors)}`);
            }
