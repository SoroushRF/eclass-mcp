...
_Pre-flight check to verify raw JSON before host-level testing._
...
_Use this template for future runs. The "Inspector Smoke Pass" was added on 2026-03-22 to ensure server stability before host testing._
...
| T41-I3 | Retry with `platformSelection.cengage.courseKey` after ambiguous candidates | `get_assignments`                | Not re-run | N/A      | Confirm `.eclass-mcp/course-platform-index.json` persists                                                   |
...
| T41-C2 | Check MATH 1014 assignments across eClass and Cengage/WebAssign.            | `get_assignments`                | Not re-run | N/A      | Missing Cengage auth should surface `/auth-cengage`                                                         |
...
| T42-I2 | `{ courseCode: "MATH1014", includeExternal: "always" }`                     | `get_assignments`                | Not re-run | N/A      | Wrong active WebAssign course returns `needs_course_activation` or `partial` with `COURSE_CONTEXT_MISMATCH` |
